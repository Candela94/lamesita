...
export const Cajas = ({ caja, isOpen, onToggle }) => {
  ...
  const ref = useRef(null); // 👈 Agrega esta ref
...
useEffect(() => {
  if (isOpen && ref.current) {
    ...
    }, 100); // Pequeño delay para que la animación de apertura se vea
  }
}, [isOpen]);
  ...
  const handleGuardar = () => {
    ...
    // Validar si hay productos con tipos
        ...
        {isOpen && !overlay && !pasoFinal && (
          ...
          >
            ...
            >
              ...
              <motion.div className="caja-info">
...
export const CajaPersonalizada = ({ caja, isOpen, onToggle }) => {
  ...
  const handleSeleccion = (productoNombre, tipo) => {
    ...
    if (esBase) {
      // Si es la base, simplemente actualiza
      ...
      setSeleccion((prev) => {
        ...
        // Si ya tiene 6 productos y está intentando agregar uno nuevo
